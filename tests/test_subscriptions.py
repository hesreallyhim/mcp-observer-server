...
async def test_list_subscriptions(server: FileMonitorMCPServer, subscription: str, temp_dir: Path):
    ...
    subscription_data = next(
        (sub for sub in response["subscriptions"] if sub["id"] == subscription),
        None
    )
