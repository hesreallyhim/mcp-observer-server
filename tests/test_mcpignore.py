...
async def test_create_mcpignore(server: FileMonitorMCPServer, temp_dir: Path):
    ...
    input_data = CreateMcpignoreInput(
        path=str(temp_dir),
...
async def test_ignore_patterns_respected(server: FileMonitorMCPServer, temp_dir: Path):
    ...
    normal_event = next(
        (change for change in changes if change["path"] == str(normal_file)),
        None
    )
    ignored_dir_event = next(
        (change for change in changes if change["path"] == str(ignored_dir)),
        None
    )
    ignored_file_event = next(
        (change for change in changes if change["path"] == str(ignored_file)),
        None
    )
    ignored_ext_event = next(
        (change for change in changes if change["path"] == str(ignored_ext_file)),
        None
    )
