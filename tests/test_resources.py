...
async def test_read_nonexistent_file_resource(
...
async def test_read_directory_resource(
    server: FileMonitorMCPServer, temp_dir: Path, test_file: Path
):
...
async def test_read_subscription_resource(
    server: FileMonitorMCPServer, subscription: str
):
