...
class Subscription(BaseModel):
    ...
    recursive: bool = Field(
        default=False, description="Whether to monitor subdirectories recursively"
    )
    ...
    ignore_patterns: List[str] = Field(
        default_factory=list, description="File patterns to exclude (glob format)"
    )
    ...
    changes: List[FileChange] = Field(
        default_factory=list, description="History of detected changes"
    )
...
class GetChangesInput(BaseModel):
    ...
    subscription_id: str = Field(
        description="ID of the subscription to get changes for"
    )
    since: Optional[str] = Field(
        default=None, description="ISO-formatted timestamp to filter changes (optional)"
    )
...
class CreateMcpignoreInput(BaseModel):
    ...
    include_defaults: bool = Field(
        default=True, description="Whether to include the default ignore patterns"
    )
...
class FileChangeHandler(FileSystemEventHandler):
    ...
    def dispatch(self, event: FileSystemEvent) -> None:
        ...
        if isinstance(event, (FileCreatedEvent, DirCreatedEvent)):
            ...
            self.logger.warning(
                f"Unknown event type {event_class}, attempting to categorize"
            )
            ...
            if hasattr(event, "is_synthetic") and event.is_synthetic:
                self.logger.debug("Synthetic event detected, checking file state")
                if os.path.exists(event_path):
                    self.logger.debug("File exists, treating as modification")
                    self.on_modified(event)
                else:
                    self.logger.debug("File doesn't exist, treating as deletion")
                    self.on_deleted(event)
            else:
                # If all else fails, use the standard dispatch
                self.logger.debug(f"Using default dispatch for {event_class}")
    ...
    def on_created(self, event: FileSystemEvent) -> None:
        ...
        # Log creation events
        event_class = event.__class__.__name__
        self.logger.debug(f"Watchdog: {event_class} detected for {str(event.src_path)}")
        ...
        if self.callback:
            ...
            if isinstance(event, (FileCreatedEvent, DirCreatedEvent)):
                ...
                self.logger.warning(
                    f"Unexpected event class {event_class} in on_created"
                )
                event_type = "created"  # Force the type for this handler

            self.logger.debug(f"Processed creation event as: {event_type}")
    ...
    def on_deleted(self, event: FileSystemEvent) -> None:
        ...
        if self.callback:
            ...
            if isinstance(event, DirDeletedEvent) and os.path.isfile(path):
                ...
                self.logger.warning(
                    f"Unexpected event class {event_class} in on_deleted"
                )
                event_type = "deleted"  # Force the type for this handler

            self.logger.debug(
                f"Processed event as: {event_type} (was originally a {event_class})"
            )
    ...
    def on_modified(self, event: FileSystemEvent) -> None:
        ...
        if self.callback:
            ...
            if isinstance(event, (FileModifiedEvent, DirModifiedEvent)):
                ...
                self.logger.warning(
                    f"Unexpected class {event_class} in on_modified handler"
                )
                event_type = "modified"  # Force the type for this handler

            self.logger.debug(f"Processed modification event as: {event_type}")
    ...
    def on_moved(self, event: FileSystemEvent) -> None:
        ...
        if self.callback:
            ...
            if hasattr(event, "src_path") and hasattr(event, "dest_path"):
                ...
                self.logger.debug(f"Processing move source as deletion: {src_path}")
                self.callback(self.subscription_id, src_path, "deleted")

                # Destination path (treat as created)
                dest_path = str(event.dest_path)
                self.logger.debug(
                    f"Processing move destination as creation: {dest_path}"
                )
                self.callback(self.subscription_id, dest_path, "created")
            else:
                self.logger.warning("Move event missing source or destination path")
...
class FileMonitorMCPServer:
    ...
    def __init__(self) -> None:
        ...
        self.observers: Dict[
            str, BaseObserver
        ] = {}  # Subscription ID -> Watchdog Observer
        self.handlers: Dict[
            str, FileChangeHandler
        ] = {}  # Subscription ID -> File Change Handler
    ...
    def _register_handlers(self) -> None:
        ...
        async def call_tool(name: str, arguments: Dict[str, Any]) -> List[TextContent]:
            ...
            match name:
                ...
                case ToolNames.CREATE_MCPIGNORE:
                    create_mcpignore_data = CreateMcpignoreInput.model_validate(
                        arguments
                    )
                    return await self._handle_create_mcpignore_tool(
                        create_mcpignore_data
                    )
        ...
        async def subscribe_resource(uri: AnyUrl) -> None:
            ...
            self.logger.info(
                f"Subscription request from {client_info} for resource: {uri_str}"
            )
            self.logger.info(
                f"🔔 NEW SUBSCRIPTION REQUEST from {client_info} for {uri_str}"
            )
            ...
            async with self.subscription_lock:
                ...
                self.logger.info(
                    f"✅ SUB: Client {client_id[:8]}... subscribed to {uri_str}"
                )
                ...
                try:
                    ...
                    self.logger.debug(
                        f"🔄 Test notification to client {client_id[:8]}..."
                    )
                    await current_session.send_resource_updated(uri=AnyUrl(uri_str))
                    self.logger.info("✅ Initial test notification sent successfully")
                except Exception as e:
                    self.logger.error(
                        f"❌ Failed to send initial test notification: {str(e)}"
                    )
                    self.logger.error(
                        f"Failed to send initial test notification: {str(e)}"
                    )
            ...
            try:
                ...
                if client_id in self.clients:
                    ...
                    self.logger.info(
                        f"Removed client {client_id[:8]}... from clients dictionary"
                    )
    ...
    def _read_mcpignore_file(self, path: str) -> List[str]:
        ...
        if os.path.isfile(mcpignore_path):
            try:
                ...
                self.logger.info(
                    f"Loaded {len(patterns)} patterns from .mcpignore file"
                )
            except Exception as e:
    ...
    async def _handle_subscribe_tool(
        self, input_data: SubscribeInput
    ) -> List[TextContent]:
        """
        ...
        if not success:
            return [
                TextContent(
                    type="text",
                    text=json.dumps({"error": f"Failed to watch path: {path}"}),
                )
            ]
        ...
        async with self.subscription_lock:
            ...
            try:
                if hasattr(self.server, "request_context") and hasattr(
                    ...
                    self.logger.info(
                        f"✅ Tool creator auto-subscribed to {subscription.id}"
                    )
            except Exception as e:
    ...
    async def _handle_unsubscribe_tool(
        self, input_data: UnsubscribeInput
    ) -> List[TextContent]:
        """
    ...
    async def _handle_create_mcpignore_tool(
        self, input_data: CreateMcpignoreInput
    ) -> List[TextContent]:
        """
        ...
        if not os.path.isdir(path):
            return [
                TextContent(
                    type="text",
                    text=json.dumps({"error": f"Path is not a directory: {path}"}),
                )
            ]
                ...
                if input_data.include_defaults:
                    ...
                    categories = {
                        "Virtual environments": [
                            ...
                            if "/venv/" in p or "/env/" in p
                        ],
                        "Package management": [
                            p
                            for p in DEFAULT_IGNORE_PATTERNS
                            if "node_modules" in p
                            or "package-lock.json" in p
                            or ".npm" in p
                        ],
                        "Python caches": [
                            p
                        ...
                        "IDE files": [
                            p
                            for p in DEFAULT_IGNORE_PATTERNS
                            if ".idea" in p or ".vscode" in p or ".vs" in p
                        ],
                        "Version control": [
                            p for p in DEFAULT_IGNORE_PATTERNS if ".git" in p
                        ],
                        "System files": [
                            p
                            for p in DEFAULT_IGNORE_PATTERNS
                        ...
                        "Logs and temp files": [
                            p
                            for p in DEFAULT_IGNORE_PATTERNS
                            if ".log" in p or ".tmp" in p or ".temp" in p
                        ],
                    }
    ...
    async def _handle_get_changes_tool(
        self, input_data: GetChangesInput
    ) -> List[TextContent]:
        """
    ...
    async def _read_subscription_resource(self, subscription_id: str) -> str:
        ...
        async with self.subscription_lock:
            ...
            subscription_data = subscription.model_dump(
                exclude={"changes", "resource_subscribers"}
            )

            # Add recent changes
            subscription_data["recent_changes"] = [
                change.model_dump()
                for change in subscription.changes[-20:]  # Show last 20 changes
            ]

            # Add subscriber count
            subscription_data["subscriber_count"] = len(
    ...
    def _handle_event(self, subscription_id: str, path: str, event_type: str) -> None:
        ...
        change = {"path": path, "event": event_type, "timestamp": time.time()}
        ...
        try:
            ...
            def done_callback(future):
                try:
                    future.result()
                    self.logger.debug(
                        f"Successfully processed {event_type} event for {path}"
                    )
                except Exception as e:
    ...
    async def _add_change_to_queue(
        self, subscription_id: str, path: str, event_type: str, change: Dict[str, Any]
    ) -> None:
        """
        ...
        while not self.stopping:
            try:
                ...
                async with self.subscription_lock:
                    ...
                    subscriber_count = len(subscription.resource_subscribers)
                    self.logger.info(
                        f"Found {subscriber_count} subscribers for notification"
                    )
                    ...
                    self.logger.info(
                        f"Notification summary: {sent_count} sent, {error_count} failed"
                    )

                # Mark this notification as done
                self.notification_queue.task_done()
                self.logger.info(
                    f"Notification processing completed for {event_type} at {path}"
                )
                ...
                self.logger.error(
                    f"Unexpected error in notification processor: {str(e)}"
                )
    ...
    def get_capabilities(
        ...
        experimental_capabilities: Dict[str, Dict[str, Any]] = dict(),
    ) -> ServerCapabilities:
        """
    ...
    async with file_monitor.lifespan():
        ...
        if monitor_path:
            try:
                ...
                if not os.path.exists(path):
                    file_monitor.logger.error(
                        f"Default monitor path does not exist: {path}"
                    )
                else:
                    # Create a subscription for the monitor path
                    file_monitor.logger.info(
                        f"Creating default subscription for {path}"
                    )
                    sub_input = SubscribeInput(path=path, recursive=True)
                    result = await file_monitor._handle_subscribe_tool(sub_input)
                    file_monitor.logger.info(
                        f"Default subscription created: {result[0].text}"
                    )
            except Exception as e:
